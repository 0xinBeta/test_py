...
while True:
    try:
        ...
        def order_size():
            ...
            return max(round(order_size, 3), 0.001)
        ...
        while True:
            ...
            while is_position_open == False:
                ...
                if long_sig == True:
                    ...
                    sl = round(price - (df['ATR'].iloc[-1] * 10), 1)
                    sl_2 = round(((ema_200 - sl) / 2) + sl, 1)
                ...
                # Open a short position
                    ...
                    ema_200 = df['EMA200'].iloc[-1]
                    sl = round(price + (df['ATR'].iloc[-1] * 10), 1)
                    sl_2 = round(((sl - ema_200) / 2) + ema_200, 1)
                    create_order_size = order_size()
            ...
            while is_position_open == True:
                ...
                if pos_side == 'long':
                    ...
                    tp = round(((entry_price - sl) / 5) + entry_price, 1)
                ...
                # Close a short position
                    ...
                    tp = round(entry_price - ((sl - entry_price) / 5), 1)
                    pos_amount = get_pos_data(symbol=symbol)
                    date_time = get_time()
                    print(
                        f'We have a {pos_side} position at {date_time} | tp:{tp} | sl:{sl}\n=========================')
